  ...
  return (
    <div>
       <h1>Inscription</h1>
        <form onSubmit={(e)=>{
            e.preventDefault();
            let gerant = {userName: userNameInput,nom:nomInput,prenom:prenomInput,age:ageInput,mdp:mdpInput,contact:contactInput,email:emailInput,sexe:sexeInput};
            console.log(gerant);
            axios
            .post("http://localhost:5000/gerant/signup", gerant)
            .then((res)=>{
              console.log(res.data);
              localStorage.setItem("gerantFullName", `${nomInput} ${prenomInput}`);
              navigate("/admin");
            })
            .catch((error)=>{
